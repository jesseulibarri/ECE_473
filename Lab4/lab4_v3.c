...
//DDRC = 0x01;
// bar graph OE_N on PORTD bits 2
DDRD = 0x04;
// encoder is on PE6 (SH/LD) and PE7 (CKINH)
// bar graph REGCLK is on PE5
...
void timer_2_init() {
	...
	// set up timer and interrupt (16Mhz / (8 * 256) = 7,812Hz = 128uS)
	// OC2 will pulse PB7 which is what the LED board PWM pin is connected to
	TCCR2 |= (1 << WGM21) | (1 << WGM20) | (1 << COM20) \
         	| (1 << COM21) | (1 << CS21); // set timer mode (PWM, 8 prescalar, inverting)
	TIMSK |= (1 << TOIE2);
	OCR2 = 0xF9;
